...
allfasta = glob.glob('/scratch/users/anniz44/ENIGMA/single_cell/protein/*.faa')
output_dir_merge = '/scratch/users/anniz44/ENIGMA/single_cell/protein/'
input_script = '/scratch/users/anniz44/scripts/single_cell/'
input_script_sub = '//scratch/users/anniz44/scripts/single_cell/annotate'
...
def annotation(all_filter_gene_fasta_file,pre_cluster = ''):
    ...
    cmds = ('%s --tblout %s.kegg.txt --cpu 40 -E %s -T %s %s %s\n') %('hmmsearch', all_filter_gene_fasta_file,cutoff,cutoff_score,database,all_filter_gene_fasta_file)
    ...
    cmds += (
                "diamond blastp --query %s --db %s.dmnd --out %s.SARG.txt --id %s --query-cover %s --outfmt 6 --max-target-seqs 2 --evalue 1e-1 --threads 40\n"
    ...
    cmds += (
                "diamond blastp --query %s --db %s.dmnd --out %s.AHR.txt --id %s --query-cover %s --outfmt 6 --max-target-seqs 2 --evalue 1e-1 --threads 40\n"
                % (all_filter_gene_fasta_file, database, all_filter_gene_fasta_file, cutoff, cutoff2))
    cutoff = 60
    cutoff2 = 80
    database = '/scratch/users/anniz44/scripts/database/Butyrate.pro.aa'
    cmds += (
                "diamond blastp --query %s --db %s.dmnd --out %s.buty.txt --id %s --query-cover %s --outfmt 6 --max-target-seqs 2 --evalue 1e-1 --threads 40\n"
                % (all_filter_gene_fasta_file, database, all_filter_gene_fasta_file, cutoff, cutoff2))
    cutoff = 50
    cutoff2 = 80
    database = '/scratch/users/anniz44/scripts/database/IntI1_database.fasta'
    cmds += (
                "diamond blastp --query %s --db %s.dmnd --out %s.int.txt --id %s --query-cover %s --outfmt 6 --max-target-seqs 2 --evalue 1e-1 --threads 40\n"
                % (all_filter_gene_fasta_file, database, all_filter_gene_fasta_file, cutoff, cutoff2))
    cutoff = 50
    cutoff2 = 80
    database = '/scratch/users/anniz44/scripts/database/SRB.AA'
    cmds += (
                "diamond blastp --query %s --db %s.dmnd --out %s.SRB.txt --id %s --query-cover %s --outfmt 6 --max-target-seqs 2 --evalue 1e-1 --threads 40\n"
                % (all_filter_gene_fasta_file, database, all_filter_gene_fasta_file, cutoff, cutoff2))
    cutoff = 0.01
...
all_fasta_set = glob.glob(os.path.join(output_dir_merge, '*.cluster.aa'))
