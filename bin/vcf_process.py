...
required.add_argument("-vcf",
                      help="path to vcf file with all mapping results (raw.vcf)",
                      type=str, default='all.raw.vcf',
                      metavar='file.raw.vcf')
required.add_argument("-snp",
                      help="path to vcf file with only SNPs(.snp.vcf)",
                      type=str, default='all.flt.snp.vcf',
                      metavar='file.flt.snp.vcf')
...
end_cutoff = 70 # contig end no SNP calling -> not used
################################################### Function ########################################################
# set up functions
...
def outputvcf(output_name):
    ...
    vcf_file_filtered.write('CHR\tPOS\tMajor_ALT\tMinor_ALT\tGenome_set\tQuality\tGene\tGene_POS\tN_or_S\tAA_change\tallele_freq\n'+''.join(vcf_file_list_freq))
    vcf_file_filtered.close()

def outputcovwindow(output_name):
    cov_output = []
    for lines in open(vcf_file.split('.flt.snp.vcf')[0] + '.raw.vcf', 'r'):
    ...
    vcf_file_filtered.write('CHR\tPOS\tavg_depth\tdepth_genome\n'+''.join(cov_output))
    vcf_file_filtered.close()
...
def SNP_check_all_fq(lines_set,CHR_old,POS_old,reference_name):
    ...
    vcf_file_list.append(
        '\t'.join(temp_snp_line) + '\t' + '\t'.join(temp_snp_line_frq) + '\t\"%s\"\t%s\t%s\t%s\n' % (
            ...
            temp_snp_line_AA))
    'CHR\tPOS\tMajor_ALT\tMinor_ALT\tGenome_set\tQuality\tGene\tGene_POS\tN_or_S\tAA_change\tallele_freq\n'
    vcf_file_list_freq.append(
        '\t'.join(temp_snp_line) + '\t' + '\t\"%s\"\t%s\t%s\t%s\t%s\n' % (
            ';'.join(str(genome_order) for genome_order in SNP), SNP_quality, '\t'.join(temp_snp_line_NS),
...
################################################### Main ########################################################
# run vcf filtering
all_vcf_file=glob.glob(args.snp)
