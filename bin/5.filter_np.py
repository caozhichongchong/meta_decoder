...
parser.add_argument('--npos', help='Randomly subsample [npos] sites from the alignment', type=int, default=0)
...
for genome in y:
    ...
    if x.shape[0] > 0 and x.shape[1] > 0:
        cov = coverage(x)
        #print coverage(x),x.shape[0],x.shape[1],args.faln*x.shape[1],(cov == 0).sum(axis=1)
    ...
    #if x.shape[0] > 0 and x.shape[1] > 0:
        #print x.shape[0],x.shape[1],(x > 0).sum(axis=2),((x > 0).sum(axis=2) > 1).sum(axis=0)
